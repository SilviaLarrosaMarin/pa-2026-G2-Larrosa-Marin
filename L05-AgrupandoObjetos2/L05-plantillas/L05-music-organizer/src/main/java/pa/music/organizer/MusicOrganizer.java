...
public class MusicOrganizer {
    ...
    private boolean validIndex(int index) {
    	...
    	if (index <= 0) {
    		System.out.println("Index " +  index + " cannot be negative or zero");
    		return false;
    	} else if (index > fileNames.size()) {
    		System.out.println("Index " +  index + " is too large");
    		return false;
    	}
    ...
    public void printFileName(int index) {
    	if (validIndex(index)) {
    		System.out.println(fileNames.get(index - 1)); // Porque java tiene base 0 y el usuario base 1, por lo que el indice 1 para el usuario sería el 2 para java
    	}
    }
    ...
    public void printAllFileNames() {
    	// Recorrer todos los índices e imprimir los nombre de fichero de cada uno
    ...
    public void printTitleName(int index) {
    	if (validIndex(index)) {
    		System.out.println(titles.get(index - 1));
    	}
    }
    ...
    public int findFirst(String searchString) {
    	for (int i = 0; i < titles.size(); i++) {
    		if (titles.get(i).contains(searchString)) {
    			return (i + 1);
    		}
    	}
    	return -1;
    ...
    public void addSong(String path, String extension, String title) {
    	String fullFileName = path + title + extension;
    	
    	if (fileNames.contains(fullFileName)) {
    		System.out.println("\nError: The file " + fullFileName + " already exists.");
    		return;
    	}
    ...
    public void loadSongs(String path, String extension, String[] nombresTemas) {
    	...
    	for (int i = 0; i < nombresTemas.length; i++) {
    		String title = nombresTemas[i];
    		addSong(path, extension, title);
    	}
